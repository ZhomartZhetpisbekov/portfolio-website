...
const ExpWidget = () => {
  ...
  const toggleJob = (job: Job) => {
    setActiveJob(job)
    console.log(activeJob.position);
  };

  return (
    <article className="flex flex-col gap-y-5 md:flex-row md:justify-center md:gap-x-4 md:py-20 md:px-0
    bg-[#141414]/80 px-5">
      ...
      <div className="flex flex-col py-5 gap-y-3 md:max-w-[30rem] md:py-0">
        ...
        <ul className="flex flex-col gap-y-3">
          {activeJob.description.map((item) => (
            <li className="relative flex items-start gap-x-2 ">
            <PaperAirplaneIcon className="absolute top-0 left 0 w-4 h-4 mt-1" />
            <p className="mx-8 left-5">
              {item}
            </p>
          </li>
          ))}
