export const experience = [
  {
    id: 1,
    position: "Frontend Developer",
    company: "ASRM",
    period: "September 2022 - January 2022",
    description: [
      "Maintained, developed, and designed the public website using Vue 2, Vuex, HTML5, and CSS",
      "Responsible for mobile responsiveness, cross-browser compatibility, registration process, and UX design      ",
      "Integrated website with RESTful API"
    ]
  },
  {
    id: 2,
    position: "Frontend Developer",
    company: "BCPD LTD.",
    period: "April 2022 - October 2022",
    description: [
      "Developed dynamic and browser-compatible pages using Next.js, TypeScript, and MUI",
      "Worked in an agile environment with weekly stand-ups to design the user UX journey",
      "Contributed to the organization of EdTech Forum"
    ]
  },
  {
    id: 3,
    position: "IELTS Trainer",
    company: "ALBUS",
    period: "April 2022 - October 2022",
    description: [
      "Developed individual lesson plans based on the course",
      "Analyzed the main methods of preparation and selected them separately for each student",
      "Conducted trial lessons for new students with an assessment of the students level of knowledge"
    ]
  }
]